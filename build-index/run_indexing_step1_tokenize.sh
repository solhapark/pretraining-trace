...
mkdir -p logs
exec > >(tee "logs/indexing_step1_${TIMESTAMP}.out")
exec 2> >(tee "logs/indexing_step1_${TIMESTAMP}.err" >&2)
...
os.chdir('/gpfs/scratch/solhapark/pretrain-trace/infini-gram/pkg')
sys.path.insert(0, '.')
...
# Load LLaMA tokenizer (same as indexing.py main() with --tokenizer llama)
import transformers
transformers.utils.logging.set_verbosity(40)
ig.tokenizer = transformers.AutoTokenizer.from_pretrained(
